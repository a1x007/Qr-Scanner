...
import com.github.alexzhirkevich.customqrgenerator.vector.style.isTransparent
...
): Drawable = QrCodeDrawableImpl0(data, options, charset)


open class QrCodeDrawableImpl0(
    data: QrData,
    ...
    private var darkPixelPaths: Path? = null
    private var lightPixelPaths: Path? = null
    private var framePaths: Path? = null
    private var framesSize: Float = 0F
...
//        }
    ...
    private fun Canvas.drawBalls(pixelSize: Float, ballPaths: Path, ballPaint: Paint) {
    ...
    private fun Canvas.highlightVersionEyes(pixelSize: Float) {
        val (frame, ball) = when (options.highlighting.versionEyes) {
            ...
            HighlightingType.Default ->
                DefaultVersionFrame0 to QrVectorBallShape.Default
        ...
        anchorCenters.forEach {
            withTranslation(
                ...
                // Draw the underlying shape first
                drawPath(createShapePath(pixelSize * 7), createShapePaint())
    ...
    private fun createShapePath(size: Float): Path {
        val path = Path()
        val radius = size / 2f // Example radius, adjust as needed
        path.addCircle(size / 2f, size / 2f, radius, Path.Direction.CW)
        return path
    }

    private fun createShapePaint(): Paint {
        return Paint().apply {
            color = Color.WHITE // Example color, adjust as needed
    ...
    private fun Canvas.drawFrames(
        ...
        frames.forEach {
            withTranslation(
                ...
                // Ensure the frame path is correct
                if (framePath.isEmpty) {
                    Log.e("DrawFrames", "Frame path is empty or not set correctly")
                ...
                // Create a clipping path to fit within the frame
                val clippingPath = Path(framePath).apply {
                    // Optionally, you can modify the clipping path if needed
                    addRect(0f, 0f, size, size, Path.Direction.CCW)
                }

                // Clip the canvas to the frame path
                clipPath(clippingPath)

                // Draw the shape
                drawPath(shapePath, shapePaint)

                // Restore the canvas to remove the clipping path
                restore()
    ...
    private fun drawToCanvas(
        ...
        val frameFillPaint = Paint().apply {
            ...
            color = Color.RED // Set your desired fill color
            isAntiAlias = true
        }
        ...
        // Create a combined path for all the areas that will exclude the background
        val exclusionPath = Path().apply {
            addPath(framePath)  // Add the frame path
            addPath(ballPath)   // Add the highlight areas
        }

        // Save the current canvas state before applying the clip
        canvas.save()

        // Clip the canvas to the exclusionPath, inverting the clip to exclude these areas
        canvas.clipPath(exclusionPath, Region.Op.DIFFERENCE)

        // Draw the background, which will now be excluded under the paths
        ...
        canvas.withTranslation(
            ...
            if (logo != null) {
                val (x1, y1) = (size - logo.width) / 2f to (size - logo.height) / 2f
                drawBitmap(logo, x1, y1, null)
            }
        }
        canvas.density = density
    }

    private fun drawToCanvas0(
        canvas: Canvas,
        size: Float,
        pixelSize: Float,
        darkPixelPath: Path,
    ...
    private fun createLogo(logoSize: Float): Bitmap? =
        if (options.logo.drawable != null) {
            ...
            ).let { if (it.isMutable) it else it.config?.let { it1 -> it.copy(it1, true) } }
                ?.applyCanvas {
                    val clip = Path().apply {
    ...
    private fun createMainElements(
        ...
        darkPixelPaths = darkPixelPath
        lightPixelPaths = lightPixelPath
                ...
                when {
                    ...
                    options.highlighting.versionEyes !is HighlightingType.None &&
                        ...
                        if (options.colors.frame is QrVectorColor.Unspecified) {
                            ...
                            val shape =
                                (if (options.highlighting.versionEyes is HighlightingType.Styled)
                                    options.shapes.frame else DefaultVersionFrame0)
    ...
    fun getDarkPixelPath(): Path? {
        return darkPixelPaths
    }

    fun getLightPixelPath(): Path? {
        return lightPixelPaths
    }
    ...
    fun getFramesSize(): Float {
        return framesSize
    }

    fun getFramesPosition(): List<Pair<Int, Int>> {
        return frames
    }
...
private object DefaultVersionFrame0 : QrVectorFrameShape {
