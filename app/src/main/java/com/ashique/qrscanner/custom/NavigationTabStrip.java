...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
    ...
    public NavigationTabStrip(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        try {
            ...
            setTitleBold(
                    typedArray.getBoolean(R.styleable.NavigationTabStrip_nts_bolds, DEFAULT_TITLE_BOLD)
            );
            setStripWeight(
            ...
            setTypeface(typedArray.getResourceId(R.styleable.NavigationTabStrip_nts_typeface,0));
            setInactiveColor(
                    typedArray.getColor(
    ...
    public void setTypeface(int fontResId) {
        try {
            Typeface tempTypeface = ResourcesCompat.getFont(getContext(), fontResId);
            setTypeface(tempTypeface);
        } catch (Exception e) {
            e.printStackTrace();
            // Set default typeface in case of failure
            setTypeface(Typeface.DEFAULT);
        }
    }

    public void setTypeface(final Typeface typeface) {
        mTypeface = typeface != null ? typeface : Typeface.DEFAULT;
